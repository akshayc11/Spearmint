...
from spearmint.pylrpredictor.curvefunctions import all_models, model_defaults
from spearmint.pylrpredictor.terminationcriterion import ConservativeTerminationCriterion
from spearmint.pylrpredictor.terminationcriterion import OptimisticTerminationCriterion
...
def run_program(cmds):
    """Run the test program.. should be obsolete.

    Runs the test program

    Arguments:
        cmds {str} -- The command to be run

    Returns:
        int -- exit code of Popen
    """
...
class Terminationcriterion(unittest.TestCase):
    ...
    def test_conservative_predict_cancel(self):
        ...
        for prob_x_greater_type in ["posterior_mean_prob_x_greater_than",
            ...
            for model_name in ["pow3", "log_power"]:
                ...
                learning_curve = y_noisy[:num_train]
                # write_xlim(xlim)
                ...
                ret = term_crit.run(learning_curve, y_best)

                print ret
                self.assertTrue(ret['terminate'])
                y_predict = ret['predictive_mean']
                ...
                self.assertFalse(ret['terminate'])
    ...
    def test_conservative_real_example(self):
        ...
        for prob_x_greater_type in ["posterior_mean_prob_x_greater_than",
            ...
            xlim = real_abort_xlim
            learning_curve = real_abort_learning_curve[:-40]
            ybest = real_abort_ybest
            xlim = len(real_abort_learning_curve)
            print len(learning_curve), xlim
            ...
            term_crit = ConservativeTerminationCriterion(
                ...
                prob_x_greater_type=prob_x_greater_type,
                predictive_std_threshold=0.005)
            ret = term_crit.run(learning_curve, ybest)
            print ret
            print real_abort_learning_curve[-1]
            self.assertTrue(ret['terminate'])
