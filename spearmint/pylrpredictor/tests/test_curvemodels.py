...
from pylrpredictor.curvemodels import CurveModel, LinearCurveModel
from pylrpredictor.curvemodels import MLCurveModel
...
class CurveModelTest(unittest.TestCase):
    ...
    def test_interface(self):
        ...
        # self.assertRaises(
        #     NotImplementedError, model.posterior_log_likelihood, x, y)
    ...
    def test_masked_mean_x_greater_than(self):
        self.assertAlmostEqual(
            0.5, masked_mean_x_greater_than([0.1, 0.9], 0.5))

        self.assertAlmostEqual(
            0.5, masked_mean_x_greater_than([0.1, 0.9, np.nan], 0.5))

        self.assertAlmostEqual(
            2. / 3., masked_mean_x_greater_than([0.1, 0.9, 0.8, np.nan], 0.5))

        self.assertAlmostEqual(
            0., masked_mean_x_greater_than([0.1, 0.1, 0.15, np.nan], 0.5))

        self.assertAlmostEqual(
            1., masked_mean_x_greater_than([0.8, 0.9, np.nan], 0.5))
