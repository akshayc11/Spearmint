...
#         print "xlim: %d" % (xlim)
#         self.xlim = xlim
#         model = setup_model_combination(xlim=xlim,
#             models=models,
#             recency_weighting=True,
#             nthreads=nthreads)
#         self.model = model
...
#         #let's made a prediction of the accuracy that will most likely be
#         #reached, that will be returned to the optimizer
...
#             return 1
#         else:
#             #we did not pass the sanity check.. let's not report this to the
#             #optimizer
#             #and pretend nothing happened
#             print "didn't pass sanity check with predicted value \
#                 %f" % y_predict
#             return 0
...
# class ConservativeTerminationCriterion(TerminationCriterion):
#     """
#         Will evaluate p(y > y_best) and stop if the result doesn't look
#         promising.
...
#             "no learning_curve.txt ... nothing to do"
...
#             #we already exceeded ybest ... let the other criterions decide
#             #when to stop
#             print "Already better than ybest... \
#                 not evaluating f(y)>f(y_best)"
#             return 0
...
#                 return self.predict()
#             else:
#                 print "predictive_std_threshold set, checking the \
#                     predictive_std first"
#                 predictive_std = self.model.predictive_std(
#                     self.xlim,
#                     thin=PREDICTION_THINNING)
...
#                     #we are gonna wait before we become more certain about
#                     #the outcome!
#                     return 0
...
# class OptimisticTerminationCriterion(TerminationCriterion):
#     """
#         Similar to the ConservativeTerminationCriterion will evaluate
#         p(y > y_best)
#         and stop if the result doesn't look promising.
#         However additionally, if the model is confident in the prediction we
#         will still
#         stop to save time, at the risk of making a wrong prediction that we
#         take.
#     """
...
#         assert predictive_std_threshold > 0
#         self.predictive_std_threshold = predictive_std_threshold
#         super(OptimisticTerminationCriterion, self).__init__(nthreads,
#               prob_x_greater_type)
...
#         predictive_std = self.model.predictive_std(self.xlim,
#             thin=PREDICTION_THINNING)
#         print "predictive_std: %f" % predictive_std
...
#             print "predictive_std high, let's check the probably \
#                 to get higher than the current ybest"
#             #we're still checking if maybe all the probability is below ybest
#             if self.prob_x_greater_type == "posterior_prob_x_greater_than":
#                 prob_gt_ybest_xlast = \
#                     self.model.posterior_prob_x_greater_than(self.xlim,
#                     ybest, thin=PREDICTION_THINNING)
#             else:
#                 prob_gt_ybest_xlast = \
#                    self.model.posterior_mean_prob_x_greater_than(self.xlim,
...
#                 return 0
#         else:
#             print "neither the predictive_std is low nor is there a ybest \
#                 ... continue"
#             return 0
