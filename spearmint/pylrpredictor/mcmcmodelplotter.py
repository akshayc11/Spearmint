...
class MCMCCurveModelPlotter(object):
    ...
    def posterior_sample_plot(self,
                              x, y=None, vline=None,
                              xaxislabel="$x$", yaxislabel="$y$", alpha=0.3,
                              label="", color="k", x_axis_scale=0.1,
                              nsamples=50, plot_ml_estimate=False,
                              ml_estimate_color="#4682b4", rasterized=False):
        ...
        for idx, theta in enumerate(samples[np.random.randint(len(samples),
                                                              size=nsamples)]):
            ...
            fill_between(x_plot, predictive_mu - sigma, predictive_mu + sigma,
                         color=color, alpha=0.01, rasterized=rasterized)

        if plot_ml_estimate:
            ml_theta = self.model.ml_curve_model.ml_params
            predictive_mu, sigma = self.model.predict_given_theta(x, ml_theta)
            plot(x_plot, predictive_mu, alpha=1.0, color=ml_estimate_color,
                 lw=3, rasterized=rasterized)
            fill_between(x_plot, predictive_mu - sigma, predictive_mu + sigma,
                         color=ml_estimate_color, alpha=0.3,
                         rasterized=rasterized)
...
class MCMCCurveModelCombinationPlotter(object):
    ...
    def __init__(self, model):
        self.model = model
        self.colors = ['r', 'g', 'b', 'y', 'cyan', 'magenta', 'Chocolate',
                       'Crimson', 'DeepSkyBlue', 'Khaki']
    ...
    def trace_plot(self, figsize=None):
        ...
        labels = ["$%s$" % (param_name) for model in self.model.fit_models
                  for param_name in model.function_params]
        titles = ["%s" % (model.name) for model in self.model.fit_models
                  for param_name in model.function_params]
        labels.append("$sigma$")
        titles.append("")
        labels.extend(["$w%d$" % idx
                       for idx in range(len(self.model.fit_models))])
        titles.extend(["%s weight" % model.name
                       for model in self.model.fit_models])
    ...
    def weights_triangle_plot(self, labels=None, thin=1):
        ...
        if labels is None:
            ...
            # labels = greek_label_mapping(labels)
            print labels
        fig = triangle.corner(samples[::thin, -len(self.model.fit_models):])
        # ,labels=labels)
        return fig
    ...
    def posterior_sample_plot(self, x, y=None, vline=None, alpha=0.1, label="",
                              xaxislabel="$x$", yaxislabel="$y$", color="k",
                              x_axis_scale=0.1, x_lim=None,
                              plot_individual=False, y_plot_lw=2,
                              rasterized=False):
        samples = self.model.get_burned_in_samples()
        ...
        for idx, theta in enumerate(samples[np.random.randint(len(samples),
                                    size=100)]):
            predictive_mu, sigma = self.model.predict_given_theta(x_predict,
                                                                  theta)
            ...
            fill_between(x_plot, predictive_mu - 2 * sigma,
                         predictive_mu + 2 * sigma, color=color,
            ...
            if self.model.normalize_weights:
                ...
                model_probs = [weight / model_weight_sum
                               for weight in model_weights]
            else:
                model_probs = model_weights
            for fit_model, model_color, model_prob, params in \
                    zip(self.model.fit_models, self.colors, model_probs,
                        model_params):
                # if fit_model.function.__name__ != "ilog2":
                #     continue
                try:
                    sub_model_predictive_mu = fit_model.function(x, *params)
                    # plot(x_plot, model_prob * sub_model_predictive_mu,
                    #      color=model_color, alpha=alpha)
                    plot(x_plot, sub_model_predictive_mu, color=model_color,
                         alpha=alpha)
                except:
    ...
    def ml_single_models_plot(self, x, y, vline=None, x_axis_scale=0.1):
        ...
        for m in self.model.fit_models:
            plot(x_plot, m.predict(x), alpha=0.7, label=m.function.__name__,
                 lw=2)
...
class MCMCCurveMixtureModelPlotter(object):
    ...
    def posterior_sample_plot(self, x, y, vline=None, alpha=0.8, label="",
                              color="k", x_axis_scale=0.1):
        samples = self.model.get_burned_in_samples()
...
class EnsemblePlotter(object):
    ...
    def __init__(self,
                 ...
                 colors=['r', 'g', 'b', 'y', 'cyan', 'magenta', 'Chocolate',
                         'Crimson', 'DeepSkyBlue', 'Khaki']):
        self.ensemble_curve_model = ensemble_curve_model
        self.colors = colors
        assert len(colors) >= len(ensemble_curve_model.all_models), \
            "Not enough colors for plot all fit models. Supply more colors!"
        # TODO: always use the same color for the same model!
        self.model_colors = {
            model.name: model_color
            for model, model_color in zip(ensemble_curve_model.all_models,
                                          colors)}

    def posterior_plot(self, x, y, vline=None, num_curves=100,
                       x_label="epochs", y_label="accuracy", x_axis_scale=0.1):
        """
            x, y: data the posterior will be plotted upon.
            num_curves: the number of curves to plot
            x_axis_scale: scale the values on the xaxis (only for plotting but
        ...
        for i in range(num_curves):
            ...
            fill_between(x_plot, predictive_mu - 2 * sigma,
                         predictive_mu + 2 * sigma, color=color, alpha=0.01)
...
class CurveModelEnsemblePlotter(EnsemblePlotter):
    ...
    def get_random_curve(self, x):
        ...
        """
        # sample model:
        model_idx = np.random.multinomial(
            1, self.ensemble_curve_model.model_probabilities).argmax()
...
class CurveEnsemblePlotter(EnsemblePlotter):
    ...
    def get_random_curve(self, x):
        ...
        model_idx = np.random.multinomial(
            1, self.ensemble_curve_model.model_probabilities).argmax()
        model = self.ensemble_curve_model.fit_models[model_idx]
        model_theta_probabilities = \
            self.ensemble_curve_model.model_theta_probabilities[model_idx]
        ...
        theta_idx = np.random.multinomial(
            1, model_theta_probabilities).argmax()
        theta = model_samples[theta_idx, :]
        params, sigma = model.split_theta(theta)
        predictive_mu = model.function(x, **params)
        return predictive_mu, sigma, model_color
