...
class CurveModelEnsemble(Ensemble):
    ...
    def fit(self, x, y, train_fraction=0.8):
        ...
        self.model_probabilities = [np.exp(log_lik - normalizing_constant)
                                    for log_lik in model_log_likelihoods]
        return True

    def posterior_prob_x_greater_than(self, x, y):
        """
            The probability under the models that a value y is exceeded at
            position x.
            IMPORTANT: if all models fail, by definition the probability is 1.0
            and NOT 0.0
        """
        if len(self.fit_models) == 0:
            return 1.0

        models_prob_x_greater_than = self._map(
            partial(model_posterior_prob_x_greater_than, x=x, y=y),
            self.fit_models)

        overall_prob = 0
        for prob_x_greater_than, model_prob in zip(models_prob_x_greater_than,
    ...
    def posterior_log_likelihood(self, x, y):
        log_liks = []
        for model, model_prob in zip(self.fit_models,
                                     self.model_probabilities):
    ...
    def __str__(self):
        ...
        for model_prob, model_name in zip(self.model_probabilities,
                                          model_names):
...
class CurveEnsemble(Ensemble):
    """
        ...
        3. Make predictions as an weighted average of those models
            The weight is the (normalized) likelihood of some held out
            validation data.
    """
    ...
    def fit(self, x, y, train_fraction=0.8):
        ...
        for model in self.fit_models:
            ...
            for theta_idx in range(thetas.shape[0]):
                ...
                model_log_likes.append(log_likelihood)
            all_log_likelihoods.append(model_log_likes)

        self.model_theta_probabilities = [
            np.exp(model_log_likelihoods - logsumexp(model_log_likelihoods))
            for model_log_likelihoods in all_log_likelihoods]

        normalizing_constant = logsumexp(all_log_likelihoods)

        def normalize(log_lik):
            return np.exp(log_lik - normalizing_constant)
        # normalize = lambda log_lik: np.exp(log_lik - normalizing_constant)

        self.all_model_probabilities = [[
            normalize(log_lik) for log_lik in model_log_likelihoods]
            for model_log_likelihoods in all_log_likelihoods]

        # sum up on a per model family basis:
        self.model_probabilities = [
            sum(model_probabilities)
            for model_probabilities in self.all_model_probabilities]
    ...
    def posterior_prob_x_greater_than(self, x, y):
        ...
        overall_prob = 0
        for model, theta_model_probabilities in zip(
                self.fit_models, self.all_model_probabilities):
            thetas = model.get_burned_in_samples()
            for theta_idx, theta_model_probability in zip(
                    range(thetas.shape[0]), theta_model_probabilities):
                theta = thetas[theta_idx, :]
                overall_prob += theta_model_probability * \
                    model.prob_x_greater_than(x, y, theta)
        return overall_prob
