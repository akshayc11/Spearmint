...
from scipy.optimize import fmin_l_bfgs_b, leastsq, nnls
...
class CurveModel(object):
    ...
    def __init__(self,
        ...
        for param_name in self.function_params:
            if param_name not in default_vals:
                print "setting function parameter %s \
                to default of 1.0 for function %s" % (param_name,
    ...
    def default_function_param_array(self):
        return np.asarray([self.default_vals[param_name]
                           for param_name in self.function_params])
    ...
    def fit_leastsq(self, x, y, weights, start_from_default):
        ...
        try:
            if weights is None:
                if self.recency_weighting:
                    residuals = \
                        get_residuals_without_weights_with_recency_weighting
                    # residuals = lambda p: np.sqrt(
                    #     recency_weights(len(y))) * (self.function(x, *p) - y)
                else:
                    residuals = \
                        get_residuals_without_weights_without_recency_weighting
                ...
                if self.recency_weighting:
                    residuals = \
                        get_residuals_with_weights_with_recency_weighting
                    # residuals = lambda p: np.sqrt(
                    #     recency_weights(len(y)) * weights) * (
                    #         self.function(x, *p) - y)
                else:
                    residuals = \
                        get_residuals_with_weights_without_recency_weighting
            ...
            popt, cov_popt, info, msg, status = leastsq(
                residuals,
                x0=initial_params,
                full_output=True)  # Dfun=, col_deriv=True)
        ...
        try:
            ...
            for param_name in self.function_params:
                if param_name in self.min_vals and param_name in self.max_vals:
                    bounds.append((self.min_vals[param_name],
                                   self.max_vals[param_name]))
            ...
            if popt is None:
                return False
            if any(np.isnan(popt)):
                logging.info("bfgs NOT successful for model %s, parameter NaN"
                             % self.name)
                return False
            sigma = self.ml_sigma(x, y, popt, weights)
            self.ml_params = np.append(popt, [sigma])
...
class MCMCCurveModel(CurveModel):
    """
        MLE curve fitting + MCMC sampling with uniform priors for parameter
        uncertainty.
    ...
    def __init__(self,
        ...
        # TODO: have two burn-ins, one for when the ML fitting is successful
        # and one for when not!
    ...
    def fit_mcmc(self, x, y):
        ...
        # pos = [start + 1e-4*np.random.randn(self.ndim)
        #        for i in range(self.nwalkers)]
        assert self.ml_curve_model.ml_params is not None
        pos = [self.ml_curve_model.ml_params + 1e-6 * np.random.randn(
            self.ndim) for i in range(self.nwalkers)]
        if self.nthreads <= 1:
    ...
    def mean_posterior_log_likelihood(self, x, y):
        ...
        return np.ma.masked_invalid(
            self.posterior_log_likelihoods(x, y)).mean()
    ...
    def dic(self, x, y):
        ...
        dic_val = 2 * np.mean(deviances) - theta_mean_deviance
        return dic_val
...
class LinearCurveModel(CurveModel):
    ...
    def fit_ml(self, x, y, weights):
        ...
        x_array = np.asarray([np.ones(len(x)), x]).T
        bh = np.dot(np.linalg.inv(np.dot(x_array.T, x_array)),
                    np.dot(x_array.T, y))
        a = bh[1]
        b = bh[0]
...
class MCMCCurveModelCombination(object):
    ...
    def __init__(
        ...
        """
            xlim: the point on the x axis we eventually want to make
            predictions for.
        """
        ...
        assert not (monotonicity_constraint and
                    soft_monotonicity_constraint), "choose either the \
        monotonicity_constraint or the soft_monotonicity_constraint, \
        but not both"
        self.monotonicity_constraint = monotonicity_constraint
        self.soft_monotonicity_constraint = soft_monotonicity_constraint
        self.initial_model_weight_ml_estimate = \
            initial_model_weight_ml_estimate
        self.normalized_weights_initialization = \
            normalized_weights_initialization
        self.strictly_positive_weights = strictly_positive_weights
        self.sanity_check_prior = sanity_check_prior
        self.nthreads = nthreads
        self.recency_weighting = recency_weighting
        # the constant used for initializing the parameters in a ball
        # around the ML parameters
        self.rand_init_ball = 1e-6
        self.name = "model combination"
        # (%s)" % ", ".join([model.name for model in self.ml_curve_models])
    ...
    def y_lim_sanity_check(self, ylim):
        ...
        # HOWEVER: there might be cases where some models might predict value
        # larger than 1.0
        # and this is alright, because in those cases we don't necessarily want
        # to stop a run.
        if not np.isfinite(ylim) or ylim < 0. or ylim > 1.0:
            return False
        else:
            return True
    ...
    def fit_ml_individual(self, x, y, model_weights):
        """
            Do a ML fit for each model individually and then another ML fit for
        ...
        for model in self.ml_curve_models:
            if model.fit(x, y):
                ...
                if not self.y_lim_sanity_check(ylim):
                    print "ML fit of model %s is out of bound range \
                        [0.0, 1.0] at xlim." % (model.function.__name__)
                    continue
        ...
        self.ml_params = self.join_theta(all_model_params, sigma,
                                         model_weights)
    ...
    def get_ml_model_weights(self, x, y_target):
        ...
        """
            ...
            Deriving and setting to zero we get a linear system of equations
    ...
    def ln_likelihood(self, theta, x, y):
        ...
        if self.recency_weighting:
            weight = recency_weights(len(y))
            ln_likelihood = (weight * norm.logpdf(y - y_model, loc=0,
                                                  scale=sigma)).sum()
        else:
            ln_likelihood = norm.logpdf(y - y_model, loc=0, scale=sigma).sum()
    ...
    def fit_mcmc(self, x, y):
        ...
        pos = [self.ml_params + self.rand_init_ball *
    ...
    def normalize_chain_model_weights(self):
        """
            In the chain we sample w_1,... w_i however we are interested in the
            model
            probabilities p_1,... p_i
        """
        model_weights_chain = self.mcmc_chain[:, :, -len(self.fit_models):]
        model_probabilities_chain = model_weights_chain /\
            model_weights_chain.sum(axis=2)[:, :, np.newaxis]
        # replace in chain
        self.mcmc_chain[:, :, -len(self.fit_models):] = \
    ...
    def predictive_distribution(self, x, thin=1):
        ...
        for theta in samples[::thin]:
            model_params, sigma, model_weights = self.split_theta(theta)
            y_predicted = self.predict_given_params(x, model_params,
                                                    model_weights)
...
class MlCurveMixtureModel(object):
    ...
    """
    # Not sure what this offset is, but kept the used and commented values
    # based on what was used, and what was commented out in the E-step
        ...
        try:
            for i in range(0, num_iter):
                ...
                for model_weight, model in zip(model_weights, fit_models):
                    responsibilities.append(self.offset + model_weight *
                ...
                for model_idx, model in enumerate(fit_models):
                    if (previous_fit_model_weights[model_idx] > 0.000001 and
                        model.fit(x, y, responsibilities[model_idx, :],
                        ...
                        model_weights.append(
                            0.01 + previous_fit_model_weights[model_idx])
                ...
                for model_weight, model in zip(model_weights, fit_models):
                    logging.debug(
                        "%s %f" % (model.function.__name__, model_weight))
...
class MCMCCurveMixtureModel(object):
    ...
    def __init__(
        ...
        assert not (monotonicity_constraint and
                    soft_monotonicity_constraint), "choose either the \
                        monotonicity_constraint or \
                        the soft_monotonicity_constraint, but not both"
        self.monotonicity_constraint = monotonicity_constraint
        self.soft_monotonicity_constraint = soft_monotonicity_constraint
        self.nthreads = nthreads
        self.recency_weighting = recency_weighting
    ...
    def fit_ml_individual(self, x, y):
        ...
        if self.ml_curve_mixture_model.fit(x, y):
            ...
            for model in self.ml_curve_models:
                if model.fit(x, y):
                    if np.isfinite(self.ln_model_prior(model,
                                                       model.ml_params)):
                        self.fit_models.append(model)
                    else:
                        print "ML fit of model %s is not supported \
                            by prior." % model.function.__name__
            model_weights = [10. for model in self.fit_models]
            if len(self.fit_models) == 0:
                return False
    ...
    def ln_likelihood(self, theta, x, y):
        ...
        ln_likelihoods = []
        for model, model_theta, model_weight in zip(self.fit_models,
                                                    model_thetas,
                                                    model_weights):
            ...
            if sample_weights is None:
                ...
                ln_likelihood += (sample_weights *
                                  norm.logpdf(y - y_model, loc=0,
                                              scale=sigma)).sum()
    ...
    def fit_mcmc(self, x, y):
        ...
        # we normalize the weights in the chain model, so that the trace plot
        # make more sense
        self.normalize_chain_model_weights()
    ...
    def prob_x_greater_than(self, x, y, theta):
        ...
        cdf = norm.cdf(y, loc=y_predicted)
        # Sigma unknown
        print "Possible source of error.. Sigma unknown at this point"
        # cdf = norm.cdf(y, loc=y_predicted, scale=sigma)
        return 1. - cdf
