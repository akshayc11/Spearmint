...
def get_options():
    ...
    if 'tasks' not in options:
        options['tasks'] = {'main': {'type': 'OBJECTIVE',
                            'likelihood': options.get('likelihood',
                                                      'GAUSSIAN')}
                            }
...
def main():
    ...
    while True:
        ...
        for resource_name, resource in resources.iteritems():
            ...
            #       resource is not full, we might wait because of cost
            #       incurred
            # Note: I chose to fill up one resource and them move on to the
            # next
...
#     # Track the time series of optimization. This should eventually go into a
#     # diagnostics module.
#     trace_fh = open(os.path.join(expt_dir, 'trace.csv'), 'a')
#     trace_fh.write("%d,%f,%d,%d,%d,%d\n"
#                    % (time.time(), best_val, best_job,
#                       tasks.pending.shape[0], tasks.invalid.shape[0],
#                       tasks.data.shape[0]))
#     trace_fh.close()

#     # Print out the best job results
#     best_job_fh = open(os.path.join(expt_dir, 'best_job_and_result.txt'),
#                        'w')
